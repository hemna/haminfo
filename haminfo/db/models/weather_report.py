...
class WeatherStation(ModelBase):
    ...
    id = sa.Column(
        sa.Integer, sa.Sequence('weather_station_id_seq'), primary_key=True, unique=True
    )
    ...
    def find_station_by_callsign(session, callsign):
        callsign = callsign.replace('\x00', '').upper()
        try:
            station = (
    ...
    def from_json(station_json):
        ...
        station = WeatherStation(
            ...
            location='POINT({} {})'.format(
                station_json['longitude'], station_json['latitude']
            ),
...
class WeatherReport(ModelBase):
    ...
    id = sa.Column(
        sa.Integer, sa.Sequence('weather_report_id_seq'), primary_key=True, unique=True
    )
    ...
    def is_valid(self):
        ...
        if (
            ...
            and self.humidity == 0
            and self.pressure == 0
            and self.wind_direction == 0
            and self.wind_speed == 0
            and self.wind_gust == 0
            and self.rain_since_midnight == 0
            and self.rain_24h == 0
            and self.rain_1h == 0
        ):
            # all values are 0
