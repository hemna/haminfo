...
# US State bounding boxes (approximate) for coordinate-based lookup
# Format: state_code: (name, min_lat, max_lat, min_lon, max_lon)
US_STATE_BOUNDS = {
    'AL': ('Alabama', 30.2, 35.0, -88.5, -84.9),
    'AK': ('Alaska', 51.2, 71.4, -179.1, -129.9),
...
def get_state_from_coords(lat: float, lon: float, country_code: str) -> tuple[str, str] | None:
...
def get_states_for_country(country_code: str) -> list[tuple[str, str]]:
    ...
    return sorted([(code, name) for code, (name, *_) in bounds.items()], key=lambda x: x[1])
